	...
	, m_deleteHiddenParts(false)
{
    ...
    importExportMenu->addAction(actionLeftp);
    importExportMenu->addAction(actionRightp);
    importExportMenu->addAction(actionRoiPS);
    importExportMenu->addAction(actionPcSep);
    BpFound->setMenu(importExportMenu);
    connect(actionLeftp,	&QAction::triggered,	this,	&ccGraphicalSegmentationTool::CutOutPpfindLeft);
    connect(actionRightp,	&QAction::triggered,	this,	&ccGraphicalSegmentationTool::CutOutPpfindRight);
    connect(actionRoiPS,	&QAction::triggered,	this,	&ccGraphicalSegmentationTool::roiAreaGet);
    connect(actionPcSep,	&QAction::triggered,	this,	&ccGraphicalSegmentationTool::roiAreaCutout);
...
bool ccGraphicalSegmentationTool::start()
{
	...
	if (!m_associatedWin)
	{
        ccLog::Warning(QString::fromLocal8Bit("[框选工具]没有被关联的窗口!"));
		return false;
	}
...
void ccGraphicalSegmentationTool::stop(bool accepted)
{
	...
	if (m_associatedWin)
	{
        m_associatedWin->displayNewMessage(QString::fromLocal8Bit("分段框选 [OFF]"),
...
bool ccGraphicalSegmentationTool::addEntity(ccHObject* entity)
{
    ...
    {
        ccLog::Warning(QString::fromLocal8Bit("[分段框选]实体[%1]在当前激活的窗口中不可见!").arg(entity->getName()));
	}
	...
	if (entity->isKindOf(CC_TYPES::POINT_CLOUD))
	{
		...
		//detect if this cloud is in fact a vertex set for at least one mesh
		{
			...
			{
                ccLog::Warning(QString::fromLocal8Bit("[分段框选]不能裁剪mesh顶点 '%1'! Select its parent mesh instead!").arg(entity->getName()));
				return false;
			}
			...
			if (cloud->filterChildren(meshes,false,CC_TYPES::MESH) != 0)
			{
				for (unsigned i=0; i<meshes.size(); ++i)
					...
					{
                        ccLog::Warning(QString::fromLocal8Bit("[Graphical Segmentation Tool] Can't segment mesh vertices '%1' directly! Select its child mesh instead!").arg(entity->getName()));
						return false;
					}
			}
		}
	...
	else if (entity->isKindOf(CC_TYPES::MESH))
	{
		...
		{
            ccLog::Warning(QString::fromLocal8Bit("[分段裁剪]primitives类型还不支持! Sorry..."));
			return false;
		}
...
void ccGraphicalSegmentationTool::roiAreaGet(){
    rxsPointCouldp lwh = {
        m_segmentationPoly->computeLength(),
        m_segmentationPoly->getWidth(),0
    };
    std::vector< rxsPointCouldp> rv;
    int pNums = m_polyVertices->size();
    for(int x = 0;x< pNums;x++){
        const CCVector3* v= m_polyVertices->getPoint(x);
        rv.push_back({v->x,v->y,v->z});
    }
    emit roiGetProcess(&rv[0],pNums,lwh,1,curOptName);
}

void ccGraphicalSegmentationTool::roiAreaCutout(){
    rxsPointCouldp lwh = {
        m_segmentationPoly->computeLength(),
        m_segmentationPoly->getWidth(),0
    };
    ...
    segmentOut();
    emit roiGetProcess(&rv[0],pNums,lwh,2,curOptName);
}

void ccGraphicalSegmentationTool::CutOutPpfindLeft(){
    rxsPointCouldp lwh = {
        m_segmentationPoly->computeLength(),
        m_segmentationPoly->getWidth(),0
    };
    ...
    segmentOut();
    QString nName = QString::fromLocal8Bit("L-") + curOptName;
    emit roiGetProcess(&rv[0],pNums,lwh,3,nName);
}

void ccGraphicalSegmentationTool::CutOutPpfindRight(){
    rxsPointCouldp lwh = {
        m_segmentationPoly->computeLength(),
        m_segmentationPoly->getWidth(),0
    };
    ...
    segmentOut();
    QString nName = QString::fromLocal8Bit("R-") + curOptName;
    emit roiGetProcess(&rv[0],pNums,lwh,4,nName);
}
...
void ccGraphicalSegmentationTool::segment(bool keepPointsInside)
{
	...
	if (!m_segmentationPoly)
    {
        ccLog::Error(QString::fromLocal8Bit("没有框选区域被选中!"));
		return;
	}

	if (!m_segmentationPoly->isClosed())
	{
        ccLog::Error(QString::fromLocal8Bit("Define and/or 请先关闭框选! (右键关闭)"));
		return;
	}
...
#pragma omp parallel for
	...
	for (QSet<ccHObject*>::const_iterator p = m_toSegment.constBegin(); p != m_toSegment.constEnd(); ++p)
	{
        ...
        curOptName = QString::fromLocal8Bit(cloud->getName().toLocal8Bit());
		assert(cloud);
...
void ccGraphicalSegmentationTool::pauseSegmentationMode(bool state)
{
	...
	if (state/*=activate pause mode*/)
	{
		...
		m_associatedWin->setInteractionMode(ccGLWindow::TRANSFORM_CAMERA());
        m_associatedWin->displayNewMessage(QString::fromLocal8Bit("分段框选 [暂停]"), ccGLWindow::UPPER_CENTER_MESSAGE, false, 3600, ccGLWindow::MANUAL_SEGMENTATION_MESSAGE);
        m_associatedWin->displayNewMessage(QString::fromLocal8Bit("解除暂停继续"), ccGLWindow::UPPER_CENTER_MESSAGE, true, 3600, ccGLWindow::MANUAL_SEGMENTATION_MESSAGE);
	}
	else
	{
		m_state = STARTED;
		...
		if (m_rectangularSelection)
		{
            m_associatedWin->displayNewMessage(QString::fromLocal8Bit("分段框选 [ON] (矩形框选)"), ccGLWindow::UPPER_CENTER_MESSAGE, false, 3600, ccGLWindow::MANUAL_SEGMENTATION_MESSAGE);
            m_associatedWin->displayNewMessage(QString::fromLocal8Bit("左键:设定起点，拖拽"), ccGLWindow::UPPER_CENTER_MESSAGE, true, 3600, ccGLWindow::MANUAL_SEGMENTATION_MESSAGE);
		}
		else
        {
            m_associatedWin->displayNewMessage(QString::fromLocal8Bit("分段框选 [ON] (多边形框选)"), ccGLWindow::UPPER_CENTER_MESSAGE, false, 3600, ccGLWindow::MANUAL_SEGMENTATION_MESSAGE);
            m_associatedWin->displayNewMessage(QString::fromLocal8Bit("左键:加入角点/右键:关闭"), ccGLWindow::UPPER_CENTER_MESSAGE, true, 3600, ccGLWindow::MANUAL_SEGMENTATION_MESSAGE);
		}
	}
...
void ccGraphicalSegmentationTool::doSetPolylineSelection()
{
    ...
    m_associatedWin->displayNewMessage(QString::fromLocal8Bit("框选分段 [ON] (矩形框选)"),ccGLWindow::UPPER_CENTER_MESSAGE,false,3600,ccGLWindow::MANUAL_SEGMENTATION_MESSAGE);
    m_associatedWin->displayNewMessage(QString::fromLocal8Bit("鼠标右键拖拽画选框"),ccGLWindow::UPPER_CENTER_MESSAGE,true,3600,ccGLWindow::MANUAL_SEGMENTATION_MESSAGE);
}
...
void ccGraphicalSegmentationTool::doExportSegmentationPolyline()
{
	...
	if (mainWindow && m_segmentationPoly)
	{
		...
		if (!mode2D)
		{
			...
			{
                hasGlobalShift = (QMessageBox::question((MainWindow*)(this->parent()), "Apply Global Shift", "At least one of the segmented entity has been shifted. Apply the same shift to the polyline?", QMessageBox::Yes, QMessageBox::No) == QMessageBox::Yes);
			}
...
//	stop(true);
    //功能修改成一键 将内容同步到对应流程中
    rxsPointCouldp lwh = {0.0f,0.0f,0.0f};
    emit roiGetProcess(0,0,lwh,0,curOptName);
}
