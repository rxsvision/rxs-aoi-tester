...
ccDBRoot::ccDBRoot(ccCustomQTreeView* dbTreeWidget, QTreeView* propertiesTreeWidget, QObject* parent) : QAbstractItemModel(parent)
{
    ...
    m_ccPropDelegate = new ccPropertiesTreeDelegate(m_propertiesModel, nullptr);
}

ccDBRoot::~ccDBRoot()
{
    if(m_ccPropDelegate!=0)
...
void ccDBRoot::addElement(ccHObject* object, bool autoExpand/*=true*/)
{
    ...
    if (!parentObject)
	{
        //如果父级为空说明是一个根对象；插入到m_treeRoot下面，并将父级设定为m_treeRoot
...
void ccDBRoot::removeElement(ccHObject* object)
{
	...
	if (!parent)
	{
        ccLog::Warning(QString::fromLocal8Bit("[removeElement]内部错误，对象没有父级对象!"));
        return;
    }
	object->prepareDisplayForRefresh();
	int childPos = parent->getChildIndex(object);
...
{
    int l = m_treeRoot->getChildrenNumber();
    if(l ==0){
        return;
    }
    for(int x = 0;x<l;x++){
        ccHObject *ccEnt = m_treeRoot->getChild(x);
        unselectEntity(ccEnt);
    }
}
...
void ccDBRoot::selectEntities(std::unordered_set<int> entIDs)
{
    ...
    {
        ccLog::Warning(QString::fromLocal8Bit("[实体选中] 没有足够的内存!"));
		return;
	}
...
//        return 0;
//    }
//    ccHObject *child = parent->getChild(0)->getChild(0);
    //当前本项目中不存在从界面选中的情况，完全可以使用确定选项
    int endpos = m_treeRoot->getChildrenNumber();
    if(endpos == 0){return 0;}
    ccHObject *child = m_treeRoot->getChild(endpos-1)->getChild(0);

    selectedEntities.push_back(child);

//	if (info)
//	{
//        info->reset();
//        info->selCount = 1;//selectedIndexes.size();
...
//		return;
    ...
    ccHObject::Container CCC;
    getSelectedEntities(CCC);
    //ccHObject* obj = static_cast<ccHObject*>(CCC[0].internalPointer());
    ccHObject* obj = CCC[0];
    if (!obj)
		return;
...
short ccDBRoot::changeSelection_my(int wich){
    QModelIndex parentIndex;
    if (!hasIndex(wich, 0, parentIndex))
    {
        return -1;
    }
    ccHObject *parent = (parentIndex.isValid() ? static_cast<ccHObject*>(parentIndex.internalPointer()) : m_treeRoot);
    ...
    ccHObject *child = parent->getChild(wich);
    child->setSelected(true);
    child->prepareDisplayForRefresh();
    //showPropertiesView(child);
    return 0;
}

void ccDBRoot::clrAllEntities(){
    int l = m_treeRoot->getChildrenNumber();
    if(l ==0){
        return;
    }
