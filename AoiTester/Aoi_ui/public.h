...
#define CBC_COLORPCUSHOW		11	//回调消息代码 显示一副带颜色的点云图
...
#define CBC_VISUAL_UPD				100//可视化流程报表结果更新
#define CBC_VISUAL_UPDSSR			101//可视化流程单步报表结果更新
#define CBC_VISUAL_ERR				200//可视化流程运行出错
#define CBC_VISUAL_WARING			201//可视化流程运行警告
...
public:
	...
	virtual void SpecParamSet(const char* pkey, float v);
	virtual void SpecParamSet(const char* pkey, int v);
...
public:
	...
	//指定已存在的目标报表为主要表进行合并单一输出
	rxsResultReport* SingleReportSpec(int tageIndex, int* fliterColsIndex, int num);
	//合并所有已经存在的表输出到一张新的表中进行单一输出
	bool SingleReportMerge(rxsResultReport& out);

	void MergeMode(bool);
	bool MergeMode();
	int Count();

	void RemapColumnHeadSet(std::string src, std::string now);
	std::string RemapColumnHeadGet(std::string src);
private:
	bool isRowFormat;
	bool mergeMode;
	bool hasClr;
	...
	std::map<std::string, std::string> chremap;
};
...
public:
	...
	rxsResultReport* operator[](const char* rrName);
private:
...
public:
	...
	void SysLogRecord(const char* msg, int errType = -1);
