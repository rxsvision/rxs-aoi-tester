...
bool ImgSrcInForm::SetParamValue(void *d){
    ...
    if(data->sType==0){
        ...
        ui->expTimes->setText(QString::number(data->inParam.lsp.ExposureTime));
    ...
    if((data->ssFlag & 1)==1){
        ...
        if(data->savePath!=0){
            ui->savePath->setText(QString::fromLocal8Bit(data->savePath));
        }else{
            ui->savePath->setText("");
        }
    }else{
...
bool ImgSrcInForm::GetParamValue(void *d){
    ImgSourceParam *data = (ImgSourceParam *)d;
    ImgSourceStep *iss = (ImgSourceStep *) (*itfper)[Curid];
    if(ui->sensor->isChecked()){
        ...
        data->inParam.lsp.ExposureTime = ui->expTimes->text().toFloat();
    }
