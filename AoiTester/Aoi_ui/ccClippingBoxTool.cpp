...
void ccClippingBoxTool::editBox()
{
    ...
    bbeDlg.setWindowTitle(QString::fromLocal8Bit("clipping盒子编辑"));
...
bool ccClippingBoxTool::addAssociatedEntity(ccHObject* entity)
{
    ...
    {
        ccLog::Error(QString::fromLocal8Bit("[裁剪盒子]没有可关联的3D视图或是无效的裁剪盒子!"));
		return false;
	}
...
void ccClippingBoxTool::removeLastContour()
{
    ...
    MainWindow* mainWindow = (MainWindow*)this->parent(); /*MainWindow::TheInstance();*/
	if (mainWindow)
	{
...
void ccClippingBoxTool::exportSlice()
{
	...
	for (unsigned ci = 0; ci != m_clipBox->getContainer().getChildrenNumber(); ++ci)
	{
		...
		if (result)
		{
            ...
            ((MainWindow*)this->parent())->addToDB(result);
            //同时导出边界区域信息
            ccBBox  box = m_clipBox->getBox();
            ((MainWindow*)this->parent())->myRoiAreaSet(result,box.maxCorner(),box.minCorner());
		}
    }
}
...
void ccClippingBoxTool::extractSlicesAndContours(bool extractSlices, bool extractContours, bool singleContourMode)
{
    ...
    ccClippingBoxRepeatDlg repeatDlg(singleContourMode, (MainWindow*)this->parent()/*MainWindow::TheInstance()*/);
	...
	if (!outputSlices.empty())
	{
        ...
        /*MainWindow::TheInstance()*/((MainWindow*)this->parent())->addToDB(sliceGroup);
	}
	...
	if (!outputContours.empty())
	{
        ...
        /*MainWindow::TheInstance()*/((MainWindow*)this->parent())->addToDB(contourGroup);
