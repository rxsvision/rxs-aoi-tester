...
public:
	...
	void ReadyParamSet(void* param) override;
	bool Gather(ImageableSensorUnit*) override;
	void* GatherResultGet(unsigned long& count) override;
...
public:
	WorkProcessDal();
	typedef enum IgnoreType { ZVALFILTER = 1, CHKRR_SAVE = 2, ONlYMMOVE = 4, AUTOEXIT = 8, TDGSRC_SAVE = 16, TD_CAL = 32, GRRTEST = 64 };
	DialGauge	DigitalDG;
	...
	/// <param name=""></param>
	/// <param name=""></param>
	void rtkInParamSet(int, u8, Movepath& mp);
	/// <summary>
	/// 清除掉所有结果测试
	/// </summary>
	void CheckResultClr();
	...
	//硬件对象获取，1-调焦镜头，2-同轴光源，3-多光谱光源
	void* HrdWareObjectGet(int index) override;
	...
	bool RoiMangerAreaSet(roiGrpMangerStep* rgms, int index, int sx, int sy, int w, int h);
	const char* RoiAreaForech(unsigned index, unsigned& id, u8& Type, rxsPointCouldp[3]);

	/// 类型:1 boundBox (max min 角点与 dm) , 
	/// 2 点域 (center ， xyz各项长度，保留),
	/// 4 首尾端点（首端点，尾端点，保留）,
	/// 8 点实体 (保留,保留,保留)
	void RoiAreaSet(const char * roiName, u8 flag, rxsPointCouldp[3], rxsPointCouldp* pdEnt, unsigned nums);
	void RoiReomve(const char* roiName);
	void RoiClr();
	roiRect RoiConvertAbstoref(int absRefIndex, int src);
	void GetLineContourMatchingPC(const char* pcFileName, int mpIndex);
	void SynchronousRoiToFlower(const char* curSelFlower);
	bool roiPcFindPoint(u8 methodType, CloudEntity& ce, rxsPointCouldp& outPoint);
	int indexFindFrom(CloudEntity& ce, rxsPointCouldp tagePoint);
	void fileCopy(const char* from, const char* toTage);
	void twoDGetDataFromPic();
	void virtualRunTester();

	void oneKeyModelTrain(std::string path);
	bool AlgorithmTester(std::string aName, std::string params);
	void ShowOpenDir(const char* fp);

	/// <summary>
	/// 点云变形度示教
	/// </summary>
	/// <param name="wich">哪一个 0模型示教，1点云示教</param>
	void PointCould_DeformTecher(u8 wich, int id, u8 dir, const char* flrn);
	void PointCould_MatchShow(const char* id, u8 dir, const char* fn);
protected:
	void SpecFileCut();
	/// <summary>
...
private:
	...
	const char* GetSaveReportFile();
	bool GetResult_OK_NG(rxsResultReport&);
	/// <summary>
	/// 移动到路径的起点位置 
	/// </summary>
	...
	short CreamFlyingRacketAndProcess(Movepath* mp, Scanpoint* scp);
	short CreamGatherAndProcess(Movepath* mp, Scanpoint* scp);
	short CreamFlowerProcess(Movepath* mp);
	...
	void TcpServerRepResultGet(int mNus);
	/// <summary>
	/// 群组运动时候处理
	/// </summary>
